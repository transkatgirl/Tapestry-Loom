...
KoboldCpp is not recommended due to a lack of request queuing and a poor implementation of logprobs (the number of requested logprobs is entirely ignored, leading to unwanted creation of single-token nodes). If you still plan on using Kobold.cpp, you will need to remove `logprobs = 1` from the inference configuration specified in the editor's menu subview.
